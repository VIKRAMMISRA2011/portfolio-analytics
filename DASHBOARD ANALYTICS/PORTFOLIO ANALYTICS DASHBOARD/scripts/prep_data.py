...
open_rows = [
    ...
    # RESOLVED against newly-provided broker P&L (see data/evidence.json):
    #  • OLA ELECTRIC — the 2,441-sh "pledged" line was an intraday round-trip (buy ₹100.25 → sell ₹100.26,
    #    01-Oct-2024); Groww's stock P&L shows every share round-tripped. Closed, +₹3,509.29 already in the record.
    #  • LODHA — Zerodha Console P&L (full period) shows it fully closed: 52 sh, +₹4,888.50, unrealised ₹0.
    #    ₹3,993.50 is in the dated cadence; the +₹895.00 tail is statement-confirmed but undated, disclosed not netted.
]
...
summary = dict(
  ...
  pending=['Combined money-weighted return (XIRR) — external cash-flow set assembled from all three broker ledgers (118 flows; ledgers conserve to the paisa); withheld pending a CDSL/demat transfer statement to resolve inter-account in-kind movements before a single capital base can be struck'],
